...
public enum Bolt11 {
    ...
    private static func parseFallbackAddress(data: Data, network: Network) -> BitcoinAddress? {
        guard data.count >= 2 else { return nil }
    ...
    private static func decodeNetwork(humanReadablePart: String) -> Network? {
        if humanReadablePart.starts(with: Prefix.forNetwork(.regtest).rawValue) {
            ...
            return .simnet
        } else if humanReadablePart.starts(with: Prefix.forNetwork(.mainnet).rawValue) {
            return .mainnet
        }
        return nil
    }
